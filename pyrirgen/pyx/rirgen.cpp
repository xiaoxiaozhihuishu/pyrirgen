...
// use [mx, my, mz, q, j, k] to define the direction of the emitted wave from the source \n"
// 2*mx + q = 0 no refel; 2*mx + q > 0 negative dir refl; 2*mx + q <0 positive dir refl  \n"
// 2*my + j = 0 no refel; 2*my + j > 0 negative dir refl; 2*my + j <0 positive dir refl  \n"
// 2*mz + k = 0 no refel; 2*mz + k > 0 negative dir refl; 2*mz + k <0 positive dir refl  \n"
// angle is the pointed angle of source
double sim_source(int mx, int my, int mz, int q, int j, int k, double* r, double* s, double* Rp_plus_Rm, double* angle, char stype) {
	if (stype == 'b' || stype == 'c' || stype == 's' || stype == 'h'){
		// Polar Pattern         rho
		// ---------------------------
		// Bidirectional         0
		// Hypercardioid         0.25    
		...
		//double last_reflec[3];
		// reflec_angle: the angle of the last reflection
		double reflec_angle[3];
		// source_image_angle: the angle of the image-mirror source. convert the angle to direction unit vector.
		double source_image_angle[3];
		source_image_angle[0] = cos(angle[1])*cos(angle[0]);
		source_image_angle[1] = cos(angle[1])*sin(angle[0]);
		source_image_angle[2] = sin(angle[1]);
		if (q == 1)
		{
			source_image_angle[0] = -source_image_angle[0];
		}
		if (j == 1)
		{
			source_image_angle[1] = -source_image_angle[1];
		}
		if (k == 1)
		{
			source_image_angle[2] = -source_image_angle[2];
		}

		int reflec_order[3];
		reflec_order[0] = 2*mx + q;
		reflec_order[1] = 2*my + j;
		reflec_order[2] = 2*mz + k;
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				//last_reflec[i] = s[i];
				reflec_angle[i] = r[i] - s[i];
			}else if ( (reflec_order[i] < 0 && reflec_order[i] % 2 ==0) || (reflec_order[i] > 0 && reflec_order[i] % 2 !=0))
			{
				//last_reflec[i] = r[i] - Rp_plus_Rm[i];
				reflec_angle[i] = - Rp_plus_Rm[i];
			}else{
				//last_reflec[i] = r[i] + Rp_plus_Rm[i];
				reflec_angle[i] =  Rp_plus_Rm[i];
			}
			gain += source_image_angle[i]*reflec_angle[i];
		}
		gain = gain/sqrt(pow(reflec_angle[0], 2) + pow(reflec_angle[1], 2) + pow(reflec_angle[2], 2));
		// 1/3 - 1
		// gain = (2+gain)/3;
		gain = rho + (1-rho) * gain;
		return gain;
...
std::vector< std::vector<double> > gen_rir(double c, double fs, const std::vector< std::vector<double> >& rr, const std::vector<double>& ss, const std::vector<double>& LL, const std::vector<double>& beta_input, const std::vector<double>& orientation, const std::vector<double>& source_orientation, int isHighPassFilter, int nDimension, int nOrder, int nSamples, char microphone_type, char source_type) {
	...
	//  source_type : [omnidirectional, subcardioid, cardioid, hypercardioid,\n"
	//				  bidirectional], default is omnidirectional.\n"
	...
	//	source_orientation : direction in which the sources are pointed, specified using\n"
	//  			  azimuth and elevation angles (in radians), default is [0 0].\n"
		...
		for (mx = -n1 ; mx <= n1 ; mx++)
		{
			...
			for (my = -n2 ; my <= n2 ; my++)
			{
				...
				for (mz = -n3 ; mz <= n3 ; mz++)
				{
					...
					for (q = 0 ; q <= 1 ; q++)
					{
						Rp_plus_Rm[0] = s[0] - r[0] + 2*q*r[0] + Rm[0];
						refl[0] = pow(beta[0], std::abs(mx)) * pow(beta[1], std::abs(mx+q));

						for (j = 0 ; j <= 1 ; j++)
						{
							Rp_plus_Rm[1] = s[1] - r[1] + 2*j*r[1] + Rm[1];
							refl[1] = pow(beta[2], std::abs(my)) * pow(beta[3], std::abs(my+j));

							for (k = 0 ; k <= 1 ; k++)
							{
								Rp_plus_Rm[2] = s[2] - r[2] + 2*k*r[2] + Rm[2];
								refl[2] = pow(beta[4],std::abs(mz)) * pow(beta[5], std::abs(mz+k));
								...
								if (std::abs(2*mx+q)+std::abs(2*my+j)+std::abs(2*mz+k) <= nOrder || nOrder == -1)
								{
									...
									if (fdist < nSamples)
									{
										...
										source_coef = sim_source(mx,my,mz,q,j,k,r,s,Rp_plus_Rm,source_angle,source_type);
										...
										for (n = 0 ; n < Tw; n++)
											if (startPosition+n >= 0 && startPosition+n < nSamples)
												imp[idxMicrophone][startPosition + n] += gain * LPI[n] * source_coef;
									}
								}
							}
						}
