...
def generateRir(roomMeasures, sourcePosition, receiverPositions, *, reverbTime=None, betaCoeffs=None, float soundVelocity=340, float fs=16000, orientation=[.0, .0], source_orientation=[.0, .0], bint isHighPassFilter=True, int nDim=3, int nOrder=-1, int nSamples=-1, micType='o', source_type='o'):
	...
	Args:
		...
		source_type (str): [omnidirectional, subcardioid, cardioid, hypercardioid, bidirectional], default is omnidirectional
		nOrder (int): reflection order, default is -1, i.e. maximum order
		nDim (int): room dimension (2 or 3), default is 3
		orientation (list[float]): direction in which the microphones are pointed, specified using azimuth and elevation angles (in radians), default is [0 0], the first 0 is positive x axis (-pi,pi], the second 0 is horizontal direction [-pi/2,pi/2]
		source_orientation (list[float]): direction in which the source is pointed, specified using azimuth and elevation angles (in radians), default is [0 0]
	...
	if all(isinstance(e, collections.abc.Iterable) for e in receiverPositions):
		multipleMics = True
	else:
		multipleMics = False
		receiverPositions = [receiverPositions]

	h = cdefs.gen_rir(soundVelocity, fs, receiverPositions, sourcePosition, roomMeasures, betaCoeffs, orientation, source_orientation, isHighPassFilter, nDim, nOrder, nSamples, ord(micType[0]), ord(source_type[0]))
